...
public class AdminDataController : ControllerBase
{
    ...
    public async Task<ActionResult<IReadOnlyList<SupporterRow>>> GetSupporters(CancellationToken ct)
    {
        try
        {
            var rows = await _db.Database.SqlQuery<SupporterRow>(
                $"""
                 SELECT TOP 500
    ...
    public async Task<ActionResult<IReadOnlyList<ResidentRow>>> GetResidents(CancellationToken ct)
    {
        try
        {
            var rows = await _db.Database.SqlQuery<ResidentRow>(
                $"""
                 SELECT
    ...
    public async Task<ActionResult<IReadOnlyList<DonationRow>>> GetDonations(CancellationToken ct)
    {
        try
        {
            var rows = await _db.Database.SqlQuery<DonationRow>(
                $"""
                 SELECT TOP 500
    ...
    public async Task<ActionResult<IReadOnlyList<SafehouseRow>>> GetSafehouses(CancellationToken ct)
    {
        try
        {
            var rows = await _db.Database.SqlQuery<SafehouseRow>(
                $"""
                 SELECT
    ...
    public async Task<ActionResult<IReadOnlyList<ActivityRow>>> GetActivities(CancellationToken ct)
    {
        try
        {
            var rows = await _db.Database.SqlQuery<ActivityRow>(
                $"""
                 SELECT TOP 80
        ...
        CancellationToken ct)
    {
        try
        {
            var rows = residentId.HasValue
                ? await _db.Database.SqlQuery<ProcessRecordingRow>(
                    $"""
                     SELECT
        ...
        CancellationToken ct)
    {
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(StatusCodes.Status500InternalServerError, new { error = "Failed to save process recording." });
        }
    }
        ...
        CancellationToken ct)
    {
        ...
        {
            var rows = residentId.HasValue
                ? await _db.Database.SqlQuery<HomeVisitationRow>(
                    $"""
                     SELECT
        ...
        CancellationToken ct)
    {
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(StatusCodes.Status500InternalServerError, new { error = "Failed to save home visitation." });
        }
    }
        ...
        CancellationToken ct)
    {
        try
        {
            var donationTrend = await _db.Database.SqlQuery<MonthAmountRow>(
                $"""
                 SELECT
            ...
            var safehousePerformance = await _db.Database.SqlQuery<SafehousePerformanceRow>(
                $"""
                 SELECT
                    sh.name AS name,
            ...
            var reintegrationOutcomes = await _db.Database.SqlQuery<LabelValueRow>(
                $"""
                 SELECT
            ...
            var servicesByQuarter = await _db.Database.SqlQuery<ServicesByQuarterRow>(
                $"""
                 WITH q AS (
            ...
            var caseCategories = await _db.Database.SqlQuery<LabelValueRow>(
                $"""
                 SELECT
            ...
            var indicators = await _db.Database.SqlQuery<IndicatorRow>(
                $"""
            ...
            {
                donationTrend = donationTrend.Select(d => new MonthAmount
                {
                    month = d.month,
                    amount = d.amount
                }).ToArray(),
                safehousePerformance = safehousePerformance.ToArray(),
                reintegrationOutcomes = reintegrationOutcomes.Select(r => new LabelValue
                {
                    label = r.label,
                    value = r.value
                }).ToArray(),
                servicesByQuarter = servicesByQuarter.ToArray(),
                caseCategories = caseCategories.Select(c => new LabelValue
                {
                    label = c.label,
                    value = c.value
                }).ToArray(),
