...
function buildCaseloadUpsertBody(data: ResidentProfile) {
  const case_subcategories = SUB_CAT_FIELDS.filter(({ key }) =>
    Boolean(data[key])
  ).map(({ label }) => label);
...
function CaseloadPage() {
  ...
  const residents = useMemo<ResidentProfile[]>(
    () =>
      residentsFromApi.map((r) => ({
        ...
        sub_cat_trafficked:
          r.case_subcategories?.includes("Trafficked") ?? false,
        sub_cat_child_labor:
          r.case_subcategories?.includes("Child Labor") ?? false,
        sub_cat_physical_abuse:
          r.case_subcategories?.includes("Physical Abuse") ?? false,
        sub_cat_sexual_abuse:
          r.case_subcategories?.includes("Sexual Abuse") ?? false,
        ...
        sub_cat_street_child:
          r.case_subcategories?.includes("Street Child") ?? false,
        sub_cat_child_with_hiv:
          r.case_subcategories?.includes("Child with HIV") ?? false,
  ...
  const filtered = useMemo(() => {
    return residents.filter((r) => {
      ...
      if (filters.safehouse && r.safehouse_id !== filters.safehouse)
        return false;
      if (filters.riskLevel && r.current_risk_level !== filters.riskLevel)
        return false;
      return true;
    });
  }, [residents, filters]);
  ...
  const criticalCount = residents.filter(
    (r) => r.current_risk_level === "Critical"
  ).length;
  ...
  function renderViewContent(r: ResidentProfile) {
    const activeSubs = SUB_CAT_FIELDS.filter((f) => r[f.key] as boolean).map(
      (f) => f.label
    );
    const socioFlags = [
      ...
      <div className="space-y-1 pb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 pt-1 pb-3">
          <ViewField
            ...
            value={
              <span
                className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-body font-medium border ${STATUS_COLORS[r.case_status as CaseStatus]}`}
              >
                {r.case_status}
              </span>
            }
          ...
          <ViewField
            ...
            value={
              <span
                className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-body font-medium border ${RISK_COLORS[r.current_risk_level as RiskLevel]}`}
              >
          ...
          <ViewField
            ...
            value={
              r.initial_risk_level ? (
                <span
                  className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-body font-medium border ${RISK_COLORS[r.initial_risk_level as RiskLevel]}`}
                >
                  {r.initial_risk_level}
                </span>
              ) : undefined
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 pt-1 pb-3">
          <ViewField
            ...
            value={r.is_pwd ? "Yes" : "No"}
          />
          {r.is_pwd && <ViewField label="PWD Type" value={r.pwd_type} />}
          <ViewField
            label="Has Special Needs"
            value={r.has_special_needs ? "Yes" : "No"}
          />
          {r.has_special_needs && (
            <ViewField label="Diagnosis" value={r.special_needs_diagnosis} />
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 pt-1 pb-3">
          <ViewField
            ...
            value={fmtDate(r.date_of_admission)}
          />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 pt-1 pb-3">
          <ViewField
            ...
            value={r.referring_agency_person}
          />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 pt-1 pb-3">
          <ViewField
            ...
            value={fmtDate(r.date_colb_registered)}
          />
          <ViewField
            label="Date COLB Obtained"
            value={fmtDate(r.date_colb_obtained)}
          />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 pt-1 pb-3">
          <ViewField
            ...
            value={r.assigned_social_worker}
          />
          <ViewField
            label="Date Case Study Prepared"
            value={fmtDate(r.date_case_study_prepared)}
          />
          <div className="col-span-2">
            <ViewField
              label="Initial Case Assessment"
              value={r.initial_case_assessment}
            />
          </div>
        </div>
    ...
    return (
      <form id="resident-form" onSubmit={handleSave} className="space-y-1 pb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3 pt-1 pb-3">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) => handleField("case_control_no", e.target.value)}
          ...
          <div className="space-y-1.5">
            ...
            <select
              aria-label="Birth status"
              value={formData.birth_status}
              onChange={(e) => handleField("birth_status", e.target.value)}
              className={selectClass()}
            >
              <option value="">Select…</option>
              {["Legitimate", "Illegitimate", "Legitimated", "Unknown"].map(
                (s) => (
                  <option key={s}>{s}</option>
                )
              )}
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) => handleField("place_of_birth", e.target.value)}
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) => handleField("religion", e.target.value)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3 pt-1 pb-3">
          ...
          <div className="space-y-1.5">
            <Label className="font-body text-xs font-semibold uppercase tracking-wide text-muted-foreground">
              Initial Risk Level <span className="text-red-500">*</span>
            </Label>
            <select
              required
              aria-label="Initial risk level"
              value={formData.initial_risk_level}
              onChange={(e) =>
                handleField("initial_risk_level", e.target.value as RiskLevel)
              }
              className={selectClass()}
            >
              {RISK_LEVELS.map((s) => (
          ...
          <div className="space-y-1.5">
            ...
            <select
              required
              aria-label="Current risk level"
              value={formData.current_risk_level}
              onChange={(e) =>
                handleField("current_risk_level", e.target.value as RiskLevel)
              }
              className={selectClass()}
            >
              {RISK_LEVELS.map((s) => (
              ...
              {SUB_CAT_FIELDS.map(({ key, label }) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      handleField(key, !checked as ResidentProfile[typeof key])
                    }
        ...
        <div className="pt-1 pb-3 space-y-3">
          ...
          {formData.is_pwd && (
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) => handleField("pwd_type", e.target.value)}
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              onChange={(e) =>
                handleField("has_special_needs", e.target.checked)
              }
              className="h-4 w-4 rounded accent-primary"
            />
            <Label
              htmlFor="has_special_needs"
              className="font-body text-sm text-foreground cursor-pointer"
            >
              Has Special Needs
            </Label>
          ...
          {formData.has_special_needs && (
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  handleField("special_needs_diagnosis", e.target.value)
                }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3 pt-1 pb-3">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) => handleField("date_of_admission", e.target.value)}
            />
          </div>
          <div className="space-y-1.5">
            <Label className="font-body text-xs font-semibold uppercase tracking-wide text-muted-foreground">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) => handleField("date_enrolled", e.target.value)}
            />
          </div>
          <div className="space-y-1.5">
            <Label className="font-body text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            ...
            <Input
              ...
              onChange={(e) => handleField("date_closed", e.target.value)}
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3 pt-1 pb-3">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) =>
                handleField("referring_agency_person", e.target.value)
              }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3 pt-1 pb-3">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) =>
                handleField("date_colb_registered", e.target.value)
              }
            />
          </div>
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) =>
                handleField("date_colb_obtained", e.target.value)
              }
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3 pt-1 pb-3">
          <div className="space-y-1.5">
            <Label className="font-body text-xs font-semibold uppercase tracking-wide text-muted-foreground">
              Assigned Social Worker <span className="text-red-500">*</span>
            </Label>
            <Input
              required
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) =>
                handleField("date_case_study_prepared", e.target.value)
              }
            />
          </div>
          <div className="col-span-2 space-y-1.5">
            ...
            <textarea
              ...
              onChange={(e) =>
                handleField("initial_case_assessment", e.target.value)
              }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3 pt-1 pb-3">
          <div className="space-y-1.5">
            ...
            <select
              aria-label="Reintegration type"
              value={formData.reintegration_type}
              onChange={(e) =>
                handleField("reintegration_type", e.target.value)
              }
              className={selectClass()}
            >
              <option value="">Select…</option>
          ...
          <div className="space-y-1.5">
            ...
            <select
              aria-label="Reintegration status"
        ...
        <div className="bg-card rounded-2xl border border-border overflow-hidden shadow-sm">
          ...
          <div className="block md:hidden divide-y divide-border">
            {isResidentsLoading ? (
              ...
              <div className="py-16 text-center text-destructive font-body text-sm">
              ...
              filtered.map((r) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3 mb-2">
                    ...
                    <span
                      className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-body font-medium border flex-shrink-0 ${RISK_COLORS[r.current_risk_level as RiskLevel]}`}
                    >
                      {r.current_risk_level}
                    </span>
                  </div>
              ...
              <TableBody>
                {isResidentsLoading ? (
                  ...
                  <TableRow>
                    <TableCell
                      colSpan={8}
                      className="text-center py-16 text-destructive font-body text-sm"
                    >
                      Failed to load residents. Please refresh.
                    </TableCell>
                  </TableRow>
