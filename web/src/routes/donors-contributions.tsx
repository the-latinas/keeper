...
function DonorsPage() {
	const [activeTab, setActiveTab] = useState<"supporters" | "contributions">(
		"supporters",
	);
	...
	const [contribForm, setContribForm] =
	...
	const filteredSupporters = useMemo(() => {
		return supporters.filter((s) => {
			...
			if (
				...
				return false;
			if (supporterFilters.type && s.supporter_type !== supporterFilters.type)
				return false;
			if (supporterFilters.status && s.status !== supporterFilters.status)
				return false;
			return true;
		});
	...
	const filteredContributions = useMemo(() => {
		return contributions
			.filter((c) => {
				...
				if (contribFilters.type && c.contribution_type !== contribFilters.type)
					return false;
				if (
					contribFilters.safehouse &&
					c.allocation_safehouse !== contribFilters.safehouse
				)
					return false;
				if (
					contribFilters.program &&
					c.allocation_program !== contribFilters.program
				)
					return false;
				return true;
			})
	...
	const otherCount = contributions.filter(
		(c) => c.contribution_type !== "Monetary",
	).length;
	...
	const allocationByProgram = useMemo(() => {
		...
		contributions
			...
			.forEach((c) => {
				const key =
					c.allocation_program || c.allocation_safehouse || "Unallocated";
	...
	function supporterTotalMonetary(id: string) {
		return contributions
			.filter(
				(c) => c.supporter_id === id && c.contribution_type === "Monetary",
			)
	...
	function openView(s: Supporter) {
		setPanelSupporter(s);
		setPanelMode("view");
	}
	function openAdd() {
		setSupporterForm(EMPTY_SUPPORTER);
		setPanelSupporter(null);
		setPanelMode("add");
	}
	...
	function handleSupporterField<K extends keyof Supporter>(
		key: K,
		val: Supporter[K],
	) {
	...
	function handleContribField<K extends keyof Contribution>(
		key: K,
		val: Contribution[K],
	) {
		...
		return (
			<div className="space-y-1 pb-6">
				...
				<div className="grid grid-cols-2 gap-x-6 gap-y-3 pt-1 pb-3">
					<ViewField
						label="Full Name / Org"
						value={s.is_anonymous ? "Anonymous" : s.name}
					/>
				...
				{s.notes && (
					<>
						...
						<p className="font-body text-sm text-foreground pt-1 pb-3 leading-relaxed">
							{s.notes}
						</p>
					</>
				)}
						...
						{contribs.map((c) => (
							...
							>
								...
								<p className="font-body text-sm text-muted-foreground">
									...
									{c.contribution_type === "In-Kind" &&
										(c.item_description ||
											`Est. ${formatPHP(c.estimated_value)}`)}
									{(c.contribution_type === "Time / Volunteer" ||
										c.contribution_type === "Skills") &&
										`${c.hours}h — ${c.skill_description || c.contribution_type}`}
									{c.contribution_type === "Social Media" &&
										`${c.platform} · ${c.reach}`}
								</p>
			...
			>
				...
				<div className="grid grid-cols-2 gap-x-4 gap-y-3 pt-1 pb-3">
					<div className="col-span-2 space-y-1.5">
						...
						<Input
							...
							onChange={(e) => handleSupporterField("name", e.target.value)}
					...
					<div className="space-y-1.5">
						...
						<Input
							...
							onChange={(e) => handleSupporterField("email", e.target.value)}
					...
					<div className="space-y-1.5">
						...
						<Input
							...
							onChange={(e) => handleSupporterField("phone", e.target.value)}
					...
					<div className="col-span-2 space-y-1.5">
						...
						<Input
							value={supporterForm.organization}
							onChange={(e) =>
								handleSupporterField("organization", e.target.value)
							}
							placeholder="Company or group (if applicable)"
						/>
					</div>
					<div className="flex items-center gap-3 col-span-2 pt-1">
						<input
							...
							onChange={(e) =>
								handleSupporterField("is_anonymous", e.target.checked)
							}
				...
				<div className="grid grid-cols-2 gap-x-4 gap-y-3 pt-1 pb-3">
					<div className="space-y-1.5">
						...
						<select
							...
							onChange={(e) =>
								handleSupporterField(
									"supporter_type",
									e.target.value as SupporterType,
								)
							}
					...
					<div className="space-y-1.5">
						...
						<select
							...
							onChange={(e) =>
								handleSupporterField(
									"status",
									e.target.value as SupporterStatus,
								)
							}
							className={selectClass()}
						>
					...
					<div className="space-y-1.5">
						...
						<Input
							required
							type="date"
							value={supporterForm.joined_date}
							onChange={(e) =>
								handleSupporterField("joined_date", e.target.value)
							}
						/>
					</div>
				</div>
				...
				<div className="pt-1 pb-3 space-y-1.5">
					...
					<textarea
						...
						onChange={(e) => handleSupporterField("notes", e.target.value)}
						placeholder="Communication preferences, relationship history, etc."
			...
			<div className="bg-card rounded-2xl border border-border shadow-sm p-6">
				...
				<form onSubmit={handleContribSave} className="space-y-4">
					<div className="grid sm:grid-cols-2 gap-4">
						<div className="space-y-1.5">
							...
							<Input
								...
								onChange={(e) => handleContribField("date", e.target.value)}
							/>
						</div>
						<div className="space-y-1.5">
							<Label className="font-body text-sm font-medium text-foreground">
					...
					<div className="grid sm:grid-cols-2 gap-4">
						<div className="space-y-1.5">
							<Label className="font-body text-sm font-medium text-foreground">
								Supporter <span className="text-red-500">*</span>
							</Label>
							<select
								required
								value={contribForm.supporter_id}
								onChange={(e) => {
							...
							>
								...
								{supporters.map((s) => (
									<option key={s.id} value={s.id}>
										{s.is_anonymous ? "Anonymous Donor" : s.name}
									</option>
								))}
							</select>
						</div>
						{type === "Monetary" && (
							<div className="space-y-1.5">
								...
								<Input
									...
									onChange={(e) =>
										handleContribField("amount", Number(e.target.value))
									}
									placeholder="0"
								/>
							</div>
						...
						{type === "In-Kind" && (
							<div className="space-y-1.5">
								...
								<Input
									type="number"
									min="0"
									value={contribForm.estimated_value || ""}
									onChange={(e) =>
										handleContribField(
											"estimated_value",
						...
						{(type === "Time / Volunteer" || type === "Skills") && (
							<div className="space-y-1.5">
								...
								<Input
									...
									min="0"
									step="0.5"
									value={contribForm.hours || ""}
									onChange={(e) =>
										handleContribField("hours", Number(e.target.value))
									}
									placeholder="0"
								/>
							</div>
						...
						<div className="grid sm:grid-cols-2 gap-4">
							<div className="space-y-1.5">
								...
								<select
									...
									onChange={(e) =>
										handleContribField("payment_method", e.target.value)
									}
									className={selectClass()}
								>
							...
							<div className="space-y-1.5">
								...
								<select
									value={contribForm.campaign}
									onChange={(e) =>
										handleContribField("campaign", e.target.value)
									}
									className={selectClass()}
								>
									<option value="">Select…</option>
					...
					{type === "In-Kind" && (
						<div className="space-y-1.5">
							...
							<textarea
								required
								rows={2}
								value={contribForm.item_description}
								onChange={(e) =>
									handleContribField("item_description", e.target.value)
								}
								placeholder="Describe the donated items, quantities, and condition…"
								className={textareaClass()}
							/>
						</div>
					)}
					...
					{(type === "Time / Volunteer" || type === "Skills") && (
						<div className="space-y-1.5">
							...
							<textarea
								required
								rows={2}
								value={contribForm.skill_description}
								onChange={(e) =>
									handleContribField("skill_description", e.target.value)
								}
								placeholder={
									type === "Skills"
										? "Describe the skills provided and how they were used…"
										: "Describe activities and beneficiaries…"
								}
								className={textareaClass()}
							/>
						</div>
						...
						<div className="grid sm:grid-cols-2 gap-4">
							<div className="space-y-1.5">
								...
								<select
									...
									onChange={(e) =>
										handleContribField("platform", e.target.value)
									}
									className={selectClass()}
								>
									<option value="">Select…</option>
							...
							<div className="space-y-1.5">
								...
								<Input
									value={contribForm.reach}
									onChange={(e) => handleContribField("reach", e.target.value)}
					...
					<div className="grid sm:grid-cols-2 gap-4">
						<div className="space-y-1.5">
							<Label className="font-body text-sm font-medium text-foreground">
								Allocation — Safehouse
							</Label>
							<select
								value={contribForm.allocation_safehouse}
								onChange={(e) =>
									handleContribField("allocation_safehouse", e.target.value)
								}
								className={selectClass()}
							>
						...
						<div className="space-y-1.5">
							...
							<select
								value={contribForm.allocation_program}
								onChange={(e) =>
									handleContribField("allocation_program", e.target.value)
								}
								className={selectClass()}
							>
								<option value="">None / General</option>
					...
					{type === "Monetary" && (
						<div className="space-y-1.5">
							...
							<Input
								value={contribForm.receipt_number}
								onChange={(e) =>
									handleContribField("receipt_number", e.target.value)
								}
								placeholder="REC-YYYY-XXXX"
							/>
						</div>
					)}
					...
					<div className="space-y-1.5">
						...
						<textarea
							...
							onChange={(e) => handleContribField("notes", e.target.value)}
	...
	function contribDescription(c: Contribution) {
		...
		if (c.contribution_type === "In-Kind")
			return c.item_description
				? c.item_description.slice(0, 60) +
						(c.item_description.length > 60 ? "…" : "")
				: `Est. ${formatPHP(c.estimated_value)}`;
		if (
			c.contribution_type === "Time / Volunteer" ||
			c.contribution_type === "Skills"
		)
			return `${c.hours}h — ${(c.skill_description || "").slice(0, 50)}${(c.skill_description || "").length > 50 ? "…" : ""}`;
		if (c.contribution_type === "Social Media")
			return `${c.platform} · ${c.reach}`;
		return "—";
	}
				...
				<div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
					{[
						...
						},
						{
							...
							value: formatPHP(totalMonetary),
							sub: "All monetary contributions",
						...
						},
						{
							...
							value: otherCount,
							sub: "In-kind, time, skills, social",
							...
							<div className="flex flex-wrap gap-3 items-center">
								<div className="flex-1 min-w-[180px]">
									<Input
										...
										value={supporterFilters.search}
										onChange={(e) =>
											setSupporterFilters((f) => ({
												...f,
								...
								{(
									[
										["type", "All Types", SUPPORTER_TYPES],
										["status", "All Statuses", SUPPORTER_STATUSES],
									] as [
										keyof typeof supporterFilters,
										string,
										readonly string[],
									][]
									...
									<select
										...
										className="h-9 rounded-3xl border border-transparent bg-input/50 px-3 text-sm font-body text-foreground outline-none focus-visible:border-ring min-w-[150px]"
									>
								...
								{(supporterFilters.search ||
									supporterFilters.type ||
									supporterFilters.status) && (
									<button
										type="button"
										onClick={() =>
											setSupporterFilters({ search: "", type: "", status: "" })
										}
										className="font-body text-sm text-muted-foreground hover:text-foreground underline underline-offset-2"
									>
										Clear
									</button>
								)}
												...
												<TableCell className="font-body text-sm text-muted-foreground">
													...
													{supporterTotalMonetary(s.id) > 0 && (
														<span className="ml-1 text-foreground font-medium">
															· {formatPHP(supporterTotalMonetary(s.id))}
														</span>
													)}
												</TableCell>
					...
					<>
							...
							<div className="bg-card rounded-2xl border border-border shadow-sm p-6 mb-5">
								<div className="flex items-center gap-2 mb-4">
									...
									<h3 className="font-heading text-base font-bold text-foreground">
										Monetary Allocation by Program
									</h3>
								</div>
									...
									{allocationByProgram.map((a, i) => {
										...
										return (
											<div key={a.label} className="flex items-center gap-1.5">
												<div
													className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${colors[i % colors.length]}`}
												/>
						...
						<div className="bg-card rounded-2xl border border-border shadow-sm p-4 mb-5">
							<div className="flex flex-wrap gap-3 items-center">
								...
								<select
									value={contribFilters.type}
									onChange={(e) =>
										setContribFilters((f) => ({ ...f, type: e.target.value }))
									}
								...
								<select
									value={contribFilters.safehouse}
									onChange={(e) =>
										setContribFilters((f) => ({
											...f,
											safehouse: e.target.value,
										}))
									}
									className="h-9 rounded-3xl border border-transparent bg-input/50 px-3 text-sm font-body text-foreground outline-none focus-visible:border-ring min-w-[160px]"
								>
									...
									{filteredContributions.length === 0 ? (
										...
										filteredContributions.map((c) => (
											...
											>
												...
												<TableCell className="font-body text-sm text-foreground max-w-[220px]">
													{contribDescription(c)}
												</TableCell>
				...
				<>
					...
					<div className="fixed inset-y-0 right-0 w-[520px] bg-background border-l border-border shadow-2xl z-50 flex flex-col animate-in slide-in-from-right duration-300 ease-out">
							...
							<div>
								...
								<h2 className="font-heading text-xl font-bold text-foreground">
									{panelMode === "add"
										? "Add Supporter"
										: panelMode === "edit"
											? supporterForm.name || "Edit"
											: panelSupporter?.is_anonymous
												? "Anonymous Donor"
												: panelSupporter?.name}
								</h2>
								{panelMode !== "add" && (
						...
						<div className="flex-1 overflow-y-auto px-6 pt-4">
							{panelMode === "view" && panelSupporter
								? renderSupporterView(panelSupporter)
								: renderSupporterForm()}
						</div>
							...
							{panelMode === "view" ? (
								...
								<>
									<Button
										variant="outline"
										onClick={() =>
											panelMode === "edit" ? setPanelMode("view") : closePanel()
										}
										className="font-body px-5 h-9 rounded-xl"
									>
										Cancel
